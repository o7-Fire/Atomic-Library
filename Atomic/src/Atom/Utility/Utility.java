...
public class Utility {
    ...
    /**
     * Iterates over a collection and waits for elements to satisfy a specific condition before performing an action on them.
     * If an item can't fulfill the condition, it will hang (because there is no timeout)
     * Basically polling
     *
     * @param <T> the type of elements in the collection
     * @param collection the collection of elements to iterate over
     * @param isReady  the condition that elements must satisfy
     * @param callback the action to perform on elements that satisfy the condition
     */
    public static <T> void waitForCollection(Collection<T> collection, Filter<T> isReady, Consumer<T> callback) {

        while(!collection.isEmpty()){
            Iterator<T> iterator = collection.iterator();
            while (iterator.hasNext()){
                T next = iterator.next();
                if(isReady.accept(next)){
