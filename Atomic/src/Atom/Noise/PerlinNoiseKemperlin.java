 ...
 */
public class PerlinNoiseKemperlin implements Noise {
	...
	public PerlinNoiseKemperlin() {
		this(Atom.Utility.Random.getInt());
	}
	
	public PerlinNoiseKemperlin(int seed) {
		p = new int[B + B + 2];
	...
	private final void init(int seed) {
		...
		for (i = 0; i < B; i++) {
			...
			do {
				u = 2 * r.nextDouble() - 1;
				v = 2 * r.nextDouble() - 1;
			}while (u * u + v * v > 1 || Math.abs(u) > 2.5 * Math.abs(v) || Math.abs(v) > 2.5 * Math.abs(u) || Math.abs(
					Math.abs(u) - Math.abs(v)) < .4);
			...
			}while (u * u + v * v + w * w > 1 || Hi > 4 * Lo || Math.min(Math.abs(U - V),
